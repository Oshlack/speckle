...
vignette: >
  ...
  %\VignetteIndexEntry{speckle: statistical methods for analysing single cell 
  RNA-seq data}
...
data objects. The proportions are then arc-sin square root transformed and for 
...
that we don't need to simulation gene expression measurements; `propeller` only 
...
to visualise the data. The scater vignette goes quite deeply into quality 
...
relationships between the cells quite well. PCA1 separates cells based on 
technology, and PCA2 separates cells based on the cell line (clusters). From the 
...
these are extracted from the metadata. Upper or lower case is acceptable, but 
...
The other option is for the user to supply the cluster, sample and group 
information explicitly to the `propeller` function.

```{r}
propeller(clusters=sce_all$cluster, sample=sce_all$sample, group=sce_all$group)
```

The cell lines were mixed together in equal proportions (~0.33) and hence we 
don't expect to see significant differences between the three clusters. However,
because bootstrapping the samples doesn't incorporate enough variability 
between the samples to mimic true biological variability, we can see that the 
H1975 cluster looks significantly different between the three technologies. The 
proportion of this cell line is closer to 0.4 for CelSeq and DropSeq, and 0.34 
for the 10x data.
...
the cluster and sample information as input.

```{r,fig.height=5,fig.width=7}
props <- getTransformedProps(sce_all$cluster, sce_all$sample)
...
with two elements: `TransformedProps` and `Proportions`. These are both matrices 
with clusters/cell types in the rows and samples in the columns.
...
would highly recommend reading the `limma` manual, which is incredibly extensive
...
`eBayes` function in `limma`. When `robust = TRUE`, robust empirical Bayes 
shrinkage of the variances is performed which mitigates the effects of outlying
observations. We set `trend = FALSE` as we don't expect a mean-variance trend 
after performing our variance stabilising transformation. There may also be an
error when `trend` is set to TRUE because there is often not enough data points
to estimate the trend.

# Tips for clustering

The experimental groups are likely to contribute large sources of variation in 
the data. In the `CellBench` data the technology effect is larger than the cell
line effect. In order to cluster the data to produce meaningful cell types that
will then feed into meaningful tests for proportions, the cell types should be 
represented in as many samples as possible. Users should consider performing 
integration techniques on their single cell data, integrating on biological 
sample or perhaps experimental group. See methods such as Harmony, Liger and 
Seurat's integration technique for more information.

Cell type label assignment should not be too refined such that every sample has
unique cell types. Consider testing more broad cell type categories for more 
meaningful results, perhaps by combining clusters that are highly similar. These
refined clusters can always be explored in terms of gene expression differences 
later on. 
It may be appropriate to perform cell type assignment using methods other than
clustering, for example classification methods garnet and scPred. This allows 
the user to classify cells into known cell types, but you run the risk of 
