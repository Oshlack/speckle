...
The default of propeller is to perform the arcsin square root transformation:
```{r}
# Perform arcsin square root transformation
propeller(sce_all)
```

An alternative variance stabilising transformation is the logit transformation. 
The logit transformation doesn't compute on proportions of 0 and 1, so a 
pseudocount of 0.5 is added to the counts matrix prior to calculating the 
cell type proportions.

```{r}
# Perform logit transformation
propeller(sce_all, transform="logit")
```
...
the cluster and sample information as input. The output from `getTransformedProps` is a list with the cell type counts, transformed proportions and proportions as elements.
...
If you are interested in seeing which model best fits the data in terms of the
...
with three elements: `Counts`, `TransformedProps` and `Proportions`. These are all matrices 
