...
```{r}
library(speckle)
#source("/Users/phipsonbelinda/Documents/Rpackages/speckle/R/propeller.R")
...
because bootstrapping the samples doesn't really incorporate enough variability 
...
that is obtained by running the `getTransformedProps` function.

We have already obtained the proportions and transformed proportions when we ran
the `getTransformedProps` function. This function outputs a list object with two
elements: `TransformedProps` and `Proportions`. These are both matrices with 
clusters/cell types in the rows and samples in the columns.
...
information which corresponds to our original vs bootstrapped samples. 
